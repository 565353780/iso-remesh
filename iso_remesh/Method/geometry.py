...
def scene_surface(
    vertices: Vector3dVector, triangles: Vector3iVector
) -> RaycastingScene:
...
def triangle_mesh_by_convex_hull_of_inner_sphere(
    vertices: Union[np.ndarray, Vector3dVector],
) -> Tuple[Union[TriangleMesh, Tuple[int]]]:
    center, radius = compute_inner_sphere(vertices)
    projected_vertices = project_to_sphere(vertices, center, radius / 2.0)
...
def replace_indices(x: Union[np.ndarray, Vector3iVector], indices: np.ndarray):
    if isinstance(x, Vector3iVector):
        x = np.asarray(x)
    N = x.max()
    y = np.empty_like(x)
    for i in range(N + 1):
...
def compute_inner_sphere(
    ...
    distances_to_center = pcd.compute_point_cloud_distance(
        PointCloud(Vector3dVector(center))
    )
...
def project_to_sphere(
    vertices: Union[np.ndarray, Vector3dVector], center: np.ndarray, radius: float
) -> np.ndarray:
...
def cartesian_to_spherical_coords(
    vertices: Union[np.ndarray, Vector3dVector, Tuple],
) -> np.ndarray:
    if isinstance(vertices, Vector3dVector):
        vertices = np.asarray(vertices)
...
def interp_vertices_values_to_triangles(
    vertices: Union[np.ndarray, Vector3dVector],
    ...
    if isinstance(vertices, Vector3dVector):
        vertices = np.asarray(vertices)
    if isinstance(triangles, Vector3iVector):
    ...
    for t_index in range(T):
        ...
        interp_values[t_index] = TriPoint.barycentric_interp(
            vertices[t_indices], values[t_indices]
        )
...
def compute_triangle_normals(
    ...
    for t_index in range(T):
        t_indices = triangles[t_index]
        triangle_normals[t_index] = TriPoint.normal(
            vertices[t_indices], normalize=normalize
        )
    return triangle_normals


def compute_triangle_barycenters(
    vertices: Union[np.ndarray, Vector3dVector],
    triangles: Union[np.ndarray, Vector3iVector],
    normalize=True,
):
