...
from iso_remesh.Data.half_edge import HalfEdgeModel
from iso_remesh.Method.geometry import scene_surface, project_to_surface


class Remesher:
    def __init__(self, mesh: o3d.geometry.TriangleMesh) -> None:
        self.model = HalfEdgeModel(mesh.vertices, mesh.triangles)
        self.initial_surface = scene_surface(mesh.vertices, mesh.triangles)
        return
    ...
    def has_collinear_edges(self, h0_index: int):
        ...
        if TriPoint.is_collinear(vertices):
            return True
    ...
    def collapse_short_edges(
        self, L_low, L_high, explicit: bool = False, foldover: float = 0
    ):
        n = 0
        E = self.model.amount_of_half_edges()
        skip = []
        ...
        for h0_index in range(E):
            ...
            if foldover > 0:
                normals_pre = list(self.normal_ring(h3_index))[
                    1:-1
                ]  # exclude f0 and f1
            ...
            if foldover > 0:
                ...
                if self.has_foldover_triangles(
                    normals_pre, normals_pos, threshold=foldover
                ):
    ...
    def equalize_valences(self, sliver: bool = False, foldover: float = 0):
        ...
        for h0_index in range(E):
            ...
            if foldover > 0:
                normals_pre = (self.normal(h0_index), self.normal(h3_index))

            if sliver:
                deviation_compactness_pre = self.compactness_deviation(h0_index)
    ...
    def isotropic_remeshing(
        ...
        for m in range(iter):
            ...
            self.model.vertices = self.project_to_surface(
                new_vertices, self.initial_surface
            )
            print(f"back to surface")

    def project_to_surface(
        self, vertices: Vector3dVector, surface: RaycastingScene
    ) -> Vector3dVector:
        projected_vertices = project_to_surface(np.asarray(vertices), surface)
    ...
    def compactness_deviation(self, h_index: int):
        return sum(
            [
                1.0 - np.mean(list(self.compactness_ring(i)))
                for i in self.model.adjacent_half_edges(h_index)
            ]
        )
